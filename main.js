...
(function () {
  ...
  /* ---------- Entrada del hero (fade + blur escalonado) ---------- */
  function initHeroEnter() {
    const hero = $(".hero");
    if (!hero) return;
    const show = () => hero.classList.add("is-loaded");
    if (reduced) { show(); return; }
    // Doble rAF: deja pintar el estado inicial y luego dispara la transición
    requestAnimationFrame(() => requestAnimationFrame(show));
    setTimeout(show, 1200); // red de seguridad si algo falla
  }
  ...
  function initReveals() {
    ...
    const io = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        ...
        el.style.transitionDelay = Math.min(idx * 55, 180) + "ms";
  ...
  /* ---------- Boot ---------- */
  function boot() {
    safe(initHeroEnter, "initHeroEnter");
